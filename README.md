...
- **合并云端改动**：用 Claude 云端会话写的代码推到了 GitHub 的 `claude/*` 分支，本地落后。面板每 10 分钟 `git fetch` 一次，卡片 git 那行出现 `☁ N` 就是有 N 个提交还没合进来；点开勾选、一键合并（能快进就快进，否则生成合并提交），可选合并后推送、合并后重启项目。某个分支冲突就撤销它、接着合别的，列出冲突文件交给你处理；超过 3 天的旧分支默认不勾，不要的点「忽略」。
- **一键推送**：本地领先时卡片上的 `↑N` 就是按钮，点一下确认要推的提交列表后 `git push`（没上游自动 `-u origin`）。从不强推；远端有本地没有的提交时提示先合并。
