...
import { api, type MergeResult, type Project } from '../api'
 ...
 * 打开时先 fetch 一次；列表来自 p.git.incoming（轮询会带回最新的），勾掉的记在 skipped 里，新出现的默认勾上。
 */
export function SyncDialog({ p, onClose, onChanged, onRestart }: {
  ...
  const chosen = incoming.filter((i) => !skipped.has(i.ref))
  const merge = async () => {
  ...
  const toggle = (ref: string) => setSkipped((s) => {
    const n = new Set(s)
        ...
        <div className="mbody">
          ...
          {incoming.length === 0
            ...
            : (
              <ul className="incoming-list">
                {incoming.map((i) => (
                  <li key={i.ref} className={skipped.has(i.ref) ? 'off' : undefined}>
                    <label className="ihead">
                      <input type="checkbox" checked={!skipped.has(i.ref)} onChange={() => toggle(i.ref)} disabled={!!busy} />
          ...
          {result && (
            <div className="problems">
              ...
              {result.failed && (
                <div className="bad"><AlertTriangle /><span>
                  {short(result.failed.ref)}：{result.failed.message}
                  {result.failed.conflicts.length > 0 && <> —— 冲突文件 <span className="mono">{result.failed.conflicts.join('、')}</span>。本地没有留下半截合并，去终端里手动合并或让 Claude 处理。</>}
                  {result.failed.conflicts.length > 0 && (
                    <button type="button" className="linkbtn" onClick={() => { void api.openTerminal(p.id) }}><Terminal />在终端打开</button>
                  )}
                </span></div>
              )}
              {result.merged.length === 0 && !result.failed && <div className="ok"><CheckCircle2 /><span>都已经在本地了，没什么要合的。</span></div>}
            </div>
          )}
          ...
          <div className="mfoot">
            <span className="mute sync-hint">能快进就快进，否则生成一个合并提交；冲突会自动撤销，不留半截。</span>
            <span className="grow" />
