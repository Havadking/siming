...
- git 操作（commit / pull / 切分支）。面板只看不动，要动去终端——所以有 10.2。
  （后来开了两个口子：把云端分支合进来、把本地提交推上去，见第 11、12 节。commit / 切分支仍然不做。）
...
合并完云端改动、或者本地自己提交了，卡片上是 `↑N`，得去终端 `git push`。现在 `↑N` 本身就是按钮。

- `GitInfo.outgoing`：`ahead > 0` 且有上游时，`git log -8 --format=%s @{u}..HEAD`，要推的提交标题，新的在前。
  `↑N` 的 hover 和确认框里都列出来。
- 点 `↑N`（或 ⋯「推送到远端」、同步弹窗里「本地领先 … 推送」）→ 浏览器 `confirm` 列出要推的提交；
  有未提交改动时提醒「不会被推送」，`behind > 0` 时提醒「多半会被拒绝，先合并」。
- `POST /api/projects/{id}/git/push` → `push_branch`：有上游 `git push`；没有上游 `git push -u origin <branch>`；
  没 origin、detached 直接报错。**从不 `--force`**。远端有本地没有的提交时 git 拒绝（`[rejected]` / `non-fast-forward` /
  `fetch first`），翻成「远端有本地没有的提交，被拒绝了——先把云端改动合进来再推」，409。
- 和 fetch / merge 共用同一个 cwd 锁。成功后卡片底下一行绿字「已推送到 origin/main」，4 秒后消失；失败红字 8 秒。
- 合并弹窗里「合并后推送到 origin」走的也是 `push_branch`。
